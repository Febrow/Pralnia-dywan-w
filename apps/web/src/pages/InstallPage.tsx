...
export default function InstallPage({ onDone }: Props) {
  ...
  // Krok 3: dane MySQL (wymagane dla wariantu PHP/shared hosting)
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-2xl">
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Host" value={dbHost} onChange={setDbHost} placeholder="np. mysql.serwer.seohost.pl albo localhost" />
            ...
            <NavButtons onBack={() => setStep(2)} onNext={() => setStep(4)} disabled={!dbHost || !dbName || !dbUser} />
          </div>
        )}
        ...
        {step === 4 && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-slate-500">Opcjonalne. Możesz pominąć i ustawić później w panelu Właściciela.</p>
            <div className="grid grid-cols-2 gap-3">
        ...
        {step === 6 && (
          <div className="space-y-4">
            ...
            {error && <div className="text-red-600 text-sm">{error}</div>}
            <NavButtons onBack={() => setStep(5)} onNext={onFinish} nextLabel={submitting ? 'Instaluję…' : 'Zainstaluj'} />
          </div>
        )}
      </div>
    </div>
