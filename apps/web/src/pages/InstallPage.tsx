  ...
  function StepIndicator() {
    return (
      <div className="flex gap-2 justify-center mb-6">
        {[1, 2, 3, 4, 5].map((n) => (
          <div key={n} className={`w-8 h-2 rounded-full ${step >= n ? 'bg-brand-600' : 'bg-slate-200'}`} />
        ))}
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-100 grid place-items-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-2xl">
        <div className="text-center mb-2">
          <div className="text-2xl font-bold text-brand-700">Kreator instalacji</div>
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <Field label="Hasło (min. 8 znaków)" type="password" value={ownerPassword} onChange={setOwnerPassword} />
            <NavButtons onNext={() => setStep(2)} disabled={!ownerEmail || ownerPassword.length < 8 || !ownerFirstName || !ownerLastName} />
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-4">
            <h2 className="text-lg font-semibold">Krok 3: SMTP (e-mail)</h2>
            <p className="text-sm text-slate-500">Opcjonalne. Możesz pominąć i ustawić później.</p>
            <div className="grid grid-cols-2 gap-3">
        ...
        {step === 4 && (
          <div className="space-y-4">
            <h2 className="text-lg font-semibold">Krok 4: SMS</h2>
            <p className="text-sm text-slate-500">
              Domyślnie aktywujemy „Console" — SMSy są tylko logowane w konsoli. Wybierz dostawcę później,
              gdy podłączysz API SMS.
            </p>
            <div className="grid grid-cols-2 gap-3">
              <label className="block">
                <span className="text-sm text-slate-600">Dostawca</span>
                <select value={smsProvider} onChange={(e) => setSmsProvider(e.target.value)} className="mt-1 w-full rounded-lg border-slate-300 border px-3 py-2">
                  <option value="console">Console (logi)</option>
                  <option value="smsapi">SMSAPI.pl</option>
                  <option value="serwersms">SerwerSMS</option>
        ...
        {step === 5 && (
          <div className="space-y-4">
            <h2 className="text-lg font-semibold">Krok 5: Pula kodów QR</h2>
            <p className="text-sm text-slate-500">
              System wygeneruje pulę unikalnych kodów do nadrukowania na naklejkach. Możesz później dogenerować więcej.
            </p>
            <Field label="Liczba kodów" type="number" value={String(qrPoolSize)} onChange={(v) => setQrPoolSize(Number(v))} />
            {error && <div className="text-red-600 text-sm">{error}</div>}
            <NavButtons onBack={() => setStep(4)} onNext={onFinish} nextLabel={submitting ? 'Instaluję…' : 'Zainstaluj'} />
          </div>
        )}
      </div>
    </div>
...
function Field({ label, value, onChange, type = 'text', placeholder }: { label: string; value: string; onChange: (v: string) => void; type?: string; placeholder?: string }) {
  return (
    <label className="block">
      ...
      <input value={value} onChange={(e) => onChange(e.target.value)} type={type} placeholder={placeholder} className="mt-1 w-full rounded-lg border-slate-300 border px-3 py-2 focus:ring-2 focus:ring-brand-500" />
    </label>
  );
}
...
function NavButtons({ onBack, onNext, disabled, nextLabel }: { onBack?: () => void; onNext?: () => void; disabled?: boolean; nextLabel?: string }) {
  return (
    <div className="flex justify-between pt-4">
      {onBack ? (
        <button onClick={onBack} className="px-4 py-2 rounded-lg border border-slate-300 text-slate-600 hover:bg-slate-50">
          Wstecz
        </button>
      ) : <div />}
      <button onClick={onNext} disabled={disabled} className="px-4 py-2 rounded-lg bg-brand-700 text-white hover:bg-brand-800 disabled:opacity-50">
        {nextLabel ?? 'Dalej'}
      </button>
    </div>
  );
